...
[[ -x server/scripts/production-entrypoint.sh ]] || { echo "production backend entrypoint missing or not executable" >&2; exit 1; }
...
grep -q 'production-entrypoint.sh' server/Dockerfile || {
  echo "backend runtime must use production entrypoint" >&2
  exit 1
}
...
grep -q 'AI_REVISION_PROVIDER: disabled' <<<"$backend_block" || {
  echo "production AI revision must default to disabled" >&2
  exit 1
}
! grep -q 'AI_REVISION_PROVIDER: fake' <<<"$backend_block" || {
  echo "fake AI revision provider must never be used by production Compose" >&2
  exit 1
}
